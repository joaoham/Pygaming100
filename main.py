...
while running:
    ...
    if wave_manager and wave_manager.current_wave > len(wave_definitions):
        draw_text(screen, "Pressione E para avançar para o pátio", (SCREEN_WIDTH // 2 - 200, current_ground_level - 100), color=(255, 255, 0), size=36)
    ...
    if room_manager.current_room == 0 and room_manager.player_at_door(player):
        draw_text(screen, "Pressione E para entrar no castelo", (SCREEN_WIDTH // 2 - 200, current_ground_level - 100), color=(255, 255, 0), size=36)
    ...
    if room_manager.current_room == 2 and boss:
        ...
        if boss.hp <= 0 and boss.state == 'death':
            draw_text(screen, "Você venceu o Boss! Parabéns!", (SCREEN_WIDTH // 2 - 200, SCREEN_HEIGHT // 2), (0, 255, 0), size=40)
            draw_text(screen, "Pressione E para encerrar", (SCREEN_WIDTH // 2 - 150, SCREEN_HEIGHT // 2 + 50), (255, 255, 255), size=30)
            if keys[pygame.K_e]:
                running = False
    ...
    if not player.alive:
        draw_text(screen, "Você morreu! Pressione R para reiniciar", (SCREEN_WIDTH // 2 - 250, SCREEN_HEIGHT // 2), (255, 0, 0), size=40)
        if keys[pygame.K_r]:
            player = Player((SCREEN_WIDTH - 250, current_ground_level - 100))
            player.rect.bottom = current_ground_level
            player.attack_damage = {"smash": 15, "thrust": 10}
