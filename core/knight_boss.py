...
class KnightBoss(pygame.sprite.Sprite):
    def __init__(self, pos, ground_y):
        ...
        for state, (path, num_frames, cols) in self.animation_data.items():
            sheet = pygame.image.load(path).convert_alpha()
            self.animations[state] = self.load_from_sheet_grid(sheet, num_frames, cols)

        attack_sheet = pygame.image.load("assets/enemies/knight/Attacks.png").convert_alpha()
        for i in range(2):
            attack_key = f"attack_{i+1}"
            self.animations[attack_key] = self.load_from_sheet_grid(attack_sheet, 8, 8, row=i)
        ...
        # ✅ Corrigido: pega a pose de pray frame 3 pra calibrar o rect
    ...
    def animate(self):
        if self.passive:
            ...
            self.image = self.animations[self.state][self.animation_index]
            return  # ✅ Sai da função, evita animar outras coisas
    ...
    def try_attack(self, player):
        ...
        if abs(self.rect.centerx - player.rect.centerx) < self.attack_range:
            if now - self.last_attack_time > self.attack_cooldown:
                attack_choice = random.choice(list(self.attack_data.keys()))
                self.state = attack_choice
                self.current_hitbox = self.attack_data[attack_choice]["hitbox"].copy()
                self.has_hit_player = False
                self.last_attack_time = now

    def update_hitbox_position(self):
        if self.current_hitbox:
            if self.state in list(self.attack_data.keys()):
    ...
    def check_attack_collision(self, player):
        if self.state in self.attack_data and self.current_hitbox:
            if self.current_hitbox.colliderect(player.rect) and not self.has_hit_player:
                damage = self.attack_data[self.state]["damage"]
